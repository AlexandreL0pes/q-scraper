...
const getGrades = async (username, password) => {
  ...
  try {
    ...
    const notaSemestral = await page.evaluate(() => {
      let primeiraLinha = document.querySelectorAll(
      ...
      for (let i = 0; i < primeiraLinha.length; i++) {
        ...
        let nome = primeiraLinha[i]
          .querySelector('td[class="conteudoTexto"]')
          .innerText.split('-')[2]
          .split('(')[0]
          .trim();
        let descricao = proximoElemento.querySelectorAll(
          'td > table > tbody > tr:nth-child(n+2) > td:nth-child(2)',
        );
        let peso = proximoElemento.querySelectorAll(
          'td > table > tbody > tr:nth-child(n+2) > td:nth-child(3)',
        );
        let notaMaxima = proximoElemento.querySelectorAll(
          'td > table > tbody > tr:nth-child(n+2) > td:nth-child(4)',
        );
        let notaObtida = proximoElemento.querySelectorAll(
...
const login = async (page, username, password) => {
  ...
  try {
    await page.waitForSelector(
      'a[href="/qacademico/alunos/boletim/index.asp"]',
    );
  } catch (error) {
